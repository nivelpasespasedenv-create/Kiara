...
const { generateLevelUpCard, getUserTheme, getUserCardName } = require('../utils/levelUpCard');
                ...
                if (result && result.leveled) {
                    ...
                    if (target) {
                        try {
                            ...
                            const themeName = getUserTheme(message.author.id);
                            const customName = getUserCardName(message.author.id);
                            const cardBuffer = await generateLevelUpCard(message.author, result.newLevel, totalXP, pct, themeName, customName);
